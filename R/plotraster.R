# Plots a \code{Rhr*} object.
# Everything below was taken from the raster package and will be removed as soon as the plot function in the raster package is exportet.

plotRaster <- function(object, col=rev(terrain.colors(25)), add = FALSE, main, ext = NULL, 
                       xlim, ylim,
                       maxpixels=100000, axes=TRUE, xlab='', ylab='', asp = 1, ...) {
	...
	z <- t(raster::as.matrix(object)[object@nrows:1,])
	if (nrow(z) == 1 | ncol(z) == 1) z <- t(z)
	z[is.infinite(z)] <- NA

	.imageplot(x, y, z, col=col, axes=axes, xlab=xlab, ylab=ylab, asp=asp, main=main, ...)	
}
