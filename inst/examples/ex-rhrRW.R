...
hist(rhrTrackStats(walk)$len)

## Three different sigmas
walk <- rhrRW(n = 3000, sigma = rep(c(1, 5, 1), each = 1000))
plot(walk)
hist(rhrTrackStats(walk[1:1000, ])$len)
hist(rhrTrackStats(walk[1001:2000, ])$len)
hist(rhrTrackStats(walk[2001:3000, ])$len)
...
b <- rhrTrajSplit(walk, rep(1:5, each = 200))
